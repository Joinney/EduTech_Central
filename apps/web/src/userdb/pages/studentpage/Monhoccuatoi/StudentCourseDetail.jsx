...
// Hàm format ngày giờ hiển thị tiếng Việt
...
export default function StudentCourseDetail({ course, onBack }) {
  ...
  // 🚀 NẾU ĐANG TRONG PHÒNG HỌP LIVE MEET -> RENDER TRANG RIÊNG BIỆT
  if (isInMeeting) {
    return (
      <LiveMeetingRoom
        course={course}
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 space-y-4">
        <div className="flex flex-col md:flex-row justify-between gap-4 items-start border-b border-slate-100 pb-4">
          ...
          <button
            ...
            className="inline-flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90 text-white text-xs font-bold rounded-xl shadow-md shadow-blue-500/20 transition-all cursor-pointer active:scale-95"
          >
            ...
            <div key={lesson.id} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5 mb-1">Thời gian: {formatDateTime(lesson.duration)}</p>
          ...
          {quizzes.map(quiz => (
            <div key={quiz.id} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-500">Thời gian: {formatDateTime(quiz.duration)} • {quiz.totalQuestions || quiz.total_questions} câu trắc nghiệm</p>
