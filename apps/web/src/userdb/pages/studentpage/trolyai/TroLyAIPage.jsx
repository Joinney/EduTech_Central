...
>>>>>>> 4070cbbc0b9ec0611d0d9c4ec0049c93f43b83d7
  ...
  // ✨ TÍNH NĂNG MỚI: State và Ref quản lý thu âm giọng nói (Voice to Text)
...
>>>>>>> 4070cbbc0b9ec0611d0d9c4ec0049c93f43b83d7
  ...
  // ✨ TÍNH NĂNG MỚI: Hàm xử lý Voice to Text
  ...
  const handleSendMessage = async (textToSend, customHistory = null) => {
    ...
    const baseMessages = customHistory !== null ? customHistory : messages;

    isAutoScrollEnabled.current = true;
    setMessages([...baseMessages, userDisplayMessage, initialAssistantMessage]);
    setInputMessage("");
    setAttachedImage(null);
    setAttachedDoc(null);
    setLoading(true);
...
>>>>>>> 4070cbbc0b9ec0611d0d9c4ec0049c93f43b83d7
  ...
  const handleEditSubmit = (msgIndex) => {
    if (!editInputValue.trim()) return;
    const historySlice = messages.slice(0, msgIndex);
    setEditingMessageId(null);
    handleSendMessage(editInputValue, historySlice);
  };
  ...
  const handleRegenerate = () => {
    ...
    const historySlice = messages.slice(0, lastUserIndex);

    setAttachedImage(lastUserMessage.image || null);
    setAttachedDoc(lastUserMessage.doc || null);
    
    handleSendMessage(lastUserMessage.content, historySlice);
  };
...
<<<<<<< HEAD
      {/* Input File ẩn. Thuộc tính capture="environment" cho phép mở máy ảnh trực tiếp */}
=======
>>>>>>> 4070cbbc0b9ec0611d0d9c4ec0049c93f43b83d7
      <input
        type="file"
...
<<<<<<< HEAD
          ...
          <div className="shrink-0 w-full bg-white border-t border-slate-100 px-4 sm:px-6 pb-4 md:pb-6 pt-2">
            <div className="max-w-3xl mx-auto space-y-2 relative">
              <div className="rounded-3xl border border-slate-200/90 shadow-sm hover:border-slate-300 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all bg-white px-3 sm:px-4 pt-2.5 sm:pt-3 pb-2 flex flex-col space-y-1.5 sm:space-y-2">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5 mt-1 border-t border-slate-100 px-1">
                  <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-slate-400">
                    ...
                    <div className="relative" ref={modelDropdownRef}>
                      ...
                      {/* Menu xổ lên (pop-up) */}
                      {isModelDropdownOpen && (
                        <div className="absolute bottom-full left-0 mb-2 w-72 bg-white rounded-2xl border border-slate-200 shadow-xl p-2 z-50 space-y-1 animate-in fade-in slide-in-from-bottom-2 duration-150">
                          <div className="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                            Chọn mô hình ({availableModels.length} models)
                          </div>
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all cursor-pointer shadow-xs ${
                        loading || uploadingFile || (!inputMessage.trim() && !attachedImage && !attachedDoc)
                          ? "bg-slate-200 text-slate-400"
                          : "bg-slate-900 hover:bg-black text-white"
                      }`}
                    >
                      {loading ? <Square className="w-3.5 h-3.5 fill-current" onClick={handleStopGenerating} /> : <ArrowUp className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                    </button>
                  </div>
                </div>
              </div>

              <p className="text-[9px] sm:text-[10px] text-slate-400 text-center select-none px-2 leading-tight mt-2">
        ...
        /* ========================================================================= */
        /* 🟢 GIAO DIỆN 2: CHAT TRỰC TIẾP (Toàn màn hình)                            */
        /* ========================================================================= */
        <div className="flex flex-col h-full w-full bg-white relative">
          <header className="h-14 border-b border-slate-200/70 px-6 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md z-10">
            <div className="flex items-center space-x-3">
              ...
              {/* Model Dropdown trong Chat View (Dự phòng ở Header) */}
          ...
          <div className="flex-1 flex overflow-hidden relative">
            {/* KHUNG TRÒ CHUYỆN (Toàn bộ phần trung tâm) */}
              ...
              >
                <div className="max-w-3xl mx-auto space-y-6 pb-20">
                  {messages.map((m, index) => (
                    <div
                      key={m.id}
                      className={`flex gap-3.5 group ${m.role === "user" ? "justify-end" : "justify-start"} animate-in fade-in duration-300`}
                    >
                      {m.role === "assistant" && (
                      ...
                      <div className="max-w-[88%] flex flex-col items-end">
                        ...
                        {/* Khung Edit Tin Nhắn (Người Dùng) */}
                        {editingMessageId === m.id ? (
                          <div className="bg-slate-50 border-2 border-blue-400 p-3 rounded-2xl w-full shadow-sm">
                            <textarea
                            ...
                            <div className="flex justify-end gap-2 mt-2">
                              ...
                              <button
                                ...
                                className="px-3 py-1 rounded-lg text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition shadow-sm"
                              >
                                Lưu & Gửi lại
                              </button>
                            </div>
                          ...
                          // View Tin Nhắn Bình Thường
                          <div
                            className={`rounded-3xl p-4.5 space-y-2 relative ${
                              m.role === "user"
                          ...
                          >
                            {/* Nút Edit (Pencil) của User, hiển thị khi Hover */}
                            {m.role === "user" && !loading && (
                              <button
                                onClick={() => {
                                  setEditingMessageId(m.id);
                                  setEditInputValue(m.content);
                                }}
                                className="absolute -left-10 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-blue-600 hover:bg-blue-50 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                            ...
                            {m.role === "assistant" && m.content && (
                              <div className="flex items-center justify-between pt-2.5 mt-2 border-t border-slate-100 text-[11px] text-slate-400">
                                ...
                                <div className="flex gap-1">
                                  ...
                                  {/* Nút Regenerate (chỉ hiện cho tin nhắn AI cuối cùng) */}
                                  {!loading && index === messages.length - 1 && (
                                    <button
                                      type="button"
                                      onClick={handleRegenerate}
                                      className="flex items-center space-x-1 hover:text-blue-600 px-2 py-1 rounded-lg hover:bg-blue-50 transition-colors cursor-pointer"
...
<<<<<<< HEAD
              ...
              {showScrollButton && (
                <button
                  ...
                  className="absolute bottom-28 left-1/2 -translate-x-1/2 p-2 bg-white/90 backdrop-blur-sm border border-slate-200 shadow-lg rounded-full text-slate-600 hover:text-blue-600 hover:bg-slate-50 transition-all z-30 cursor-pointer animate-in slide-in-from-bottom-5"
                >
              ...
              <div className="p-4 bg-white border-t border-slate-100 relative">
                ...
                {/* Nút Dừng Trả Lời (Stop Generating) nổi lên trên Input */}
                {loading && (
                  <div className="absolute -top-12 left-0 right-0 flex justify-center animate-in slide-in-from-bottom-2 z-20">
                    <button
                      onClick={handleStopGenerating}
                      className="flex items-center space-x-2 px-4 py-2 bg-white border border-slate-200 shadow-lg rounded-full text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-all cursor-pointer"
                    >
...
>>>>>>> 4070cbbc0b9ec0611d0d9c4ec0049c93f43b83d7
                    ...
                    {/* KHU VỰC ĐÃ ĐƯỢC CHỈNH SỬA: Đưa Menu chọn Model vào thanh chat bên dưới */}
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5 border-t border-slate-100 mt-1 px-1">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-slate-400">
                        <button
                          type="button"
                          onClick={() => docInputRef.current?.click()}
...
<<<<<<< HEAD
                      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                        {/* ✨ Nút Nhập Giọng Nói (Voice) đã được xử lý */}
                        <button
                          type="button"
                          title="Nhập giọng nói"
                          onClick={handleVoiceInput}
                        ...
                        >
                          {/* Thay đổi icon Mũi tên thành Vuông (Dừng) nếu đang loading */}
                          {loading ? <Square className="w-3 h-3 fill-current" onClick={handleStopGenerating} /> : <ArrowUp className="w-4 h-4" />}
                        </button>
                      </div>
=======
                      <button
...
<<<<<<< HEAD
      ...
      {/* ========================================================================= */}
      {/* 🟢 MODAL: DANH SÁCH LỊCH SỬ PHIÊN HỌC (MONGODB)                             */}
      {/* ========================================================================= */}
      {showHistoryModal && (
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {savedSessions.length === 0 ? (
                <p className="text-center text-xs text-slate-400 py-6">Chưa có lịch sử trò chuyện nào.</p>
              ) : (
                savedSessions.map((s) => (
                  <div
                    key={s.id}
                    onClick={() => {
                      ...
                      const loadedMessages = s.messages.map((msg, index) => ({
                        id: `loaded-${index}`,
                        role: msg.role,
                        content: msg.content,
                        time: ""
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-400 line-clamp-2">
                      {s.messages && s.messages.length > 0 ? s.messages[0].content : "Chưa có nội dung"}
                    </p>
                  </div>
                ))
              )}
