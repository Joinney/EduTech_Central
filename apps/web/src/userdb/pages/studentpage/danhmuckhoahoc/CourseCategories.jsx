...
const MOCK_COURSES = [
  ...
  },
  {
    ...
    thumbnail:
      "https://images.unsplash.com/photo-1546410531-bb4caa6b424d?w=600&auto=format&fit=crop&q=60",
  },
];
...
export default function CourseCategories() {
  ...
  useEffect(() => {
    const fetchCoursesFromDB = async () => {
      ...
      try {
        ...
        const res = await fetch(`${baseUrl}/courses${queryParam}`).catch(
          () => null,
        );
        ...
        if (res && res.ok) {
          ...
          const fallbackRes = await courseService
            .getAllCourses()
            .catch(() => []);
        ...
        const mapped = list.map((c) => {
          ...
          const thumb =
            c.thumbnail && !c.thumbnail.includes("ui-avatars")
              ? c.thumbnail
              : defaultThumb;
  ...
  const filteredCourses = useMemo(() => {
    const keywords = categorySlug
      ? CATEGORY_KEYWORDS[categorySlug] || [categorySlug]
      : [];
      ...
      .filter((course) => {
        ...
        if (categorySlug) {
          ...
          const matchKeyword = keywords.some(
            (kw) => subjectLower.includes(kw) || titleLower.includes(kw),
          );
        ...
        if (searchTerm.trim()) {
          ...
          const matchInstructor = course.instructor
            .toLowerCase()
            .includes(query);
        ...
        {currentCategory && (
          <div className="p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start space-x-4">
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border ${currentCategory.bgLight}`}
              >
        ...
        {isLoading ? (
            ...
            {filteredCourses.map((course) => (
              ...
              >
                ...
                <div className="p-4 pt-0 mt-2 flex items-center justify-between border-t border-slate-50">
                  ...
                  >
                    {isTeacher ? "Xem giáo án" : "Xem chi tiết"}
                  </Link>
                </div>
              </div>
            ))}
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-12 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Không có khóa học nào khớp với bộ lọc hoặc từ khóa tìm kiếm của
              bạn trong danh mục này.
            </p>
            <button
              type="button"
              onClick={() => {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 font-sans pb-16">
      ...
      >
        ...
        <p className="text-sm text-slate-600 mt-1 max-w-2xl">
          Chọn một danh mục để bắt đầu khám phá các khóa học chuẩn hóa từ bậc
          phổ thông đến chuyên ngành đại học.
        </p>
      </div>
        ...
        {CATEGORIES.map((cat) => {
          ...
          const matchCount = combinedCourses.filter((c) => {
            ...
            return keywords.some(
              (kw) => sub.includes(kw) || title.includes(kw),
            );
          }).length;
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-xs transition-transform duration-300 group-hover:scale-110 ${cat.bgLight}`}
                  >
                ...
                <h3
                  className={`font-extrabold text-sm text-slate-900 group-hover:transition-colors line-clamp-1 ${
                    isTeacher
                      ? "group-hover:text-orange-600"
                      : "group-hover:text-blue-600"
                  }`}
